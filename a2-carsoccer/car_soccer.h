...
public:
	...
	Vector3 Reflection(Vector3 direction, Vector3 normal_force);
	Point3 newPosition(Point3 position, Vector3 velocity, double deltaTime);
	bool collisionWillHappen(Point3 position, Point3 newPosition);
	float distance(Point3 position1, Point3 position2);
	Point3 thresholding(Point3 vec);
	float increase_angle(float angle);
	float decrease_angle(float angle);

	bool collisionWillHappenC(Point3 position, Point3 newPosition);
	Point3 thresholdingC(Point3 vec);
